...
def fun_generate_deadline(job,SubmissionTime):
    ...
    return (SubmissionTime + random.uniform(min_time, 3 * min_time))
...
def generate_data(nInitialJ, nN, nJ, lambdaa, myseed, distribution):
...
if __name__ == '__main__':
    ...
    if (nInitialJ == 0 or nN == 0 or lambdaa == 0 or \
        (distribution != 'exponential' and distribution != 'poisson')):
        print('Sorry!! the inputs are not valuable. Data generation failed.')
    else:
        generate_data(nInitialJ, nN, nJ, lambdaa, myseed, distribution)
